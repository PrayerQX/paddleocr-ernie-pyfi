...
ARCHITECTURE_TEMPLATE = """# PaddleOCR-VL + ERNIE 4.5 Reproducible Document Intelligence Architecture
...
def export_architecture(destination: str | Path) -> Path:
    path = Path(destination)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(ARCHITECTURE_TEMPLATE, encoding="utf-8")
    return path
